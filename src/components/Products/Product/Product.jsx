...
const Product = ({ product, onAddToCart }) => {
    ...
    const addHistory =  ( product ) => {
        ...
        if ( localStorage.getItem( "lastViewed" )===null ){
            prevHistory.push( product );
            localStorage.setItem( "lastViewed", JSON.stringify( prevHistory ));
        }
        else {
            prevHistory = JSON.parse( localStorage.getItem( "lastViewed" ));
            console.log( prevHistory );

            if ( prevHistory.some( vendor => vendor.id !== product.id )){
                prevHistory.push( product );
                localStorage.setItem( "lastViewed",  JSON.stringify( prevHistory ));
            }
            else{
    ...
    return ( <>
        ...
        <div className="relative  flex flex-col items-center justify-center ">
            <div className="container">
                <div className="transform transition duration-500 hover:scale-110 max-w-md w-full  shadow-lg rounded-xl p-6">
                    <div className="flex flex-col ">
                        <div className="">
                            <div className="relative h-62 w-full mb-3">
                                ...
                                <img style={{ maxHeight: "222px" }} src={product.image.url} alt="Just a flower" className=" w-full   object-fill  rounded-2xl" />
                            </div>
                            <div className="flex-auto justify-evenly">
                                <div className="flex flex-wrap ">
                                    <div className="w-full flex-none text-sm flex items-center text-gray-600">
                                        ...
                                        <span className="whitespace-nowrap mr-3">4.60</span>
                                        {/* <span className="mr-2 text-gray-400">India</span> */}
                                    </div>
                                    <div className="flex items-center w-full justify-between min-w-0 ">
                                        <h2 className="text-lg mr-auto cursor-pointer font-extrabold	 hover:text-purple-500 truncate ">{product.name}
                                        </h2>
                                        <div className="flex font-extrabold	 items-center bg-green-400 text-white text-xs px-2 py-1 ml-3 rounded-lg">
                                            En STOCK</div>
                                    </div>
                                </div>
                                <div className="text-xl font-semibold mt-1">{product.price.formatted}</div>
                                <div className="lg:flex  py-4  text-sm">
                                    ...
                                    */}
                                    {/* <div className="flex-1 inline-flex items-center mb-3">
                                        <Typography style={{ color: "black", fontWeight: "bolder" }} dangerouslySetInnerHTML={{ __html: product.description }} variant="body2" component="p" />
                                ...
                                <div className="flex space-x-2 text-sm font-medium justify-start">
                                    ...
                                    <button   onClick={event =>  { window.location.href=`/details/${product.id}`; addHistory( product );}} className="transition ease-in duration-300 bg-gray-700 hover:bg-gray-800 border hover:border-gray-500 border-gray-700 hover:text-white  hover:shadow-lg text-gray-400 rounded-full w-9 h-9 text-center p-2">
