...
function Details  ()  {
    ...
    const handleAddToCart = async ( productId, qte ) => {
    ...
    useEffect(() => {
        ...
        myFunction();
        return () => {
            setDetails({}); // This worked for me
        };
    }, []);
        
    const myFunction = () => {
        commerce.products.retrieve( id_prod ).then(( product ) => {
                ...
                <div className="py-6">
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
                        ...
                        <div className="flex flex-col md:flex-row -mx-4">
                            ...
                            <div className="md:flex-1 px-4">
                                <h2 className="mb-2 leading-tight tracking-tight font-bold text-gray-800 text-2xl md:text-3xl"> {details.name} </h2>
                                <p className="text-gray-500 text-sm">By <a href="#" className="text-indigo-600 hover:underline">ABC Company</a></p>
  
                                <div className="flex items-center space-x-4 my-4">
                                    <div>
                                        <div className="rounded-lg bg-gray-100 flex py-2 px-3">
                                            <span className="text-indigo-400 mr-1 mt-1">DH</span>
                                            <span className="font-bold text-indigo-600 text-3xl"> {details?.price?.raw} </span>
                                ...
                                <p className="text-gray-500" dangerouslySetInnerHTML={{ __html: details.description }}></p>

  
                                <div className="flex py-4 space-x-4">
                                    <div className="relative">
                                        <div className="text-center left-0 pt-2 right-0 absolute block text-xs uppercase text-gray-400 tracking-wide font-semibold">Qte</div>
                                        <select value={quantite} onChange={handleChange} className="cursor-pointer appearance-none rounded-xl border border-gray-200 pl-4 pr-8 h-14 flex items-end pb-1">
                                            <option>1</option>
                                            <option>2</option>
                                            <option>3</option>
                                            <option>4</option>
                                    ...
                                    <button onClick={ event=>{handleAddToCart( id_prod, quantite );}} type="button" className="h-14 px-6 py-2 font-semibold rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white">
                                                Ajouter                                    
                                    </button>
                                </div>
                            </div>
